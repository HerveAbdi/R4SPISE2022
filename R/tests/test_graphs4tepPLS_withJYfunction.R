...
source("R/tests/pls.pair.plot_fromJY.R")
...
color.obs <- list()
color.obs$oc <- col4I
color.obs$gc <- as.matrix(c(group1 = "hotpink1", group2 = "#436EEE"))

color.tab <- list()
color.tab$oc[[1]] <- col4J
color.tab$oc[[2]] <- col4K
color.tab$gc[[1]] <- c("#1CBE4F", "#C4451C")
color.tab$gc[[2]] <- c("#1CFFCE", "tomato")

##
res <- list(lx = res.tepPLS$TExPosition.Data$lx,
            ly = res.tepPLS$TExPosition.Data$ly,
            p = res.tepPLS$TExPosition.Data$pdq$p,
            q = res.tepPLS$TExPosition.Data$pdq$q)

#####################################################
pls.plot <- pls.pair.plot(res,
              leDim = c(1,1),
              color.obs,
              color.tab,
              DESIGN = names(col4I),
