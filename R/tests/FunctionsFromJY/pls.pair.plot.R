pls.pair.plot <- function(res,
  ...
  lv2plot <- cbind(res$TExPosition.Data$lx[,leDim[1]],res$TExPosition.Data$ly[,leDim[2]])
  ...
  p.tab1 <- PrettyBarPlot2(res$TExPosition.Data$pdq$p[,leDim[1]],
                           threshold = 0,
                           signifOnly = sigbar.p,
  ...
  q.tab2 <- PrettyBarPlot2(res$TExPosition.Data$pdq$q[,leDim[2]],
                           threshold = 0,
                           signifOnly = sigbar.q,
  ...
  return(list(master = master, lv.plot = lv.plot, p.bar = p.tab1, q.bar = q.tab2))
