...
The inference analysis of PCA (performed by `OTAplotInference`) includes bootstrap and permutation tests of the eigenvalues and the bootstrap tests of the column factor scores (i.e., the right singular vectors scaled to have the variance of each dimension equals the corresponding singular value; $\mathbf{Q}\boldsymbol{\Delta}$ with $\mathbf{X = P}\boldsymbol{\Delta}\mathbf{Q}^\top$).
...
The bootstrap test identifies three significant dimensions that have eigenvalues reliably different from 0. The permutation test identifies one significant dimension with an eigenvalue significantly larger than 0.

### Bootstrap test on the column factor scores

The bar plot illustrates the _bootstrap ratios_ which equals $$\frac{f_j}{\sqrt{SD_{f_{j}boot}}},$$ where $f_j$ is the column factor scores of the *j*th variable and $SD_{boot}$ is the bootstrapped standard deviation of the factor score. A bootstrap ratio approximates the _t_-statistics of the column factor score with the $\mathrm{H}_0: f_j = 0$. The threshold is set to 2 to approximate the _t_-value if 1.96 at $\alpha$ = 0.05 (1.96).
